...
###############################################################################
# FUNCION: detectar_audio_spa_preferencia
# Busca el primer stream de audio español con preferencia:
# 1. AAC español
# 2. MP2 español
# 3. AC3 español
# 4. Cualquier español
# 5. El primer audio disponible
###############################################################################
...
###############################################################################
# FUNCION: get_valid_audio_map
# Obtiene el mapeo de audio válido para FFmpeg, prueba si el índice existe.
# Si el índice no es válido, usa el primer audio disponible.
###############################################################################
get_valid_audio_map() {
    local udp_url="$1"
    local idx
    ...
    idx=$(echo "$idx" | tr -d '\n')
    # Probar si el índice es válido
    ffmpeg -hide_banner -nostats -v error -i "$udp_url" -map 0:a:"$idx" -f null - 2>&1 | grep -q "Stream map '' matches no streams"
    if [[ $? -eq 0 ]]; then
        # El índice no es válido, mapea el primer audio
